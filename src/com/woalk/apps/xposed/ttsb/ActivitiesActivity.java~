...
public class ActivitiesActivity extends Activity {
	...
	protected void submitToDB() {
		new SuggestTask().execute("http://ext.woalk.de/ttsb_database/insertsuggestion.php");
	}
	...
	private class SuggestTask extends AsyncTask<String, Void, HttpResponse> {
		...
		protected HttpResponse doInBackground(String... params) {
			...
			for (int i = 0; i < lA.activities.size(); i++) {
				...
				if (activity == "" || activity == null) continue;
				if (activity == "All") activity = "";
				
				Settings.Parser parser = Settings.Loader.loadWithNull(sPref, app.packageName, activity);
				if (parser != null) {
					activities += activity;
			...
			try {
				...
				nameValuePairs.add(new BasicNameValuePair("package", app.packageName));
				nameValuePairs.add(new BasicNameValuePair("activities", activities));
				nameValuePairs.add(new BasicNameValuePair("settings", settings));
		...
		protected void onPostExecute(HttpResponse result) {
			if (result != null) 
				Toast.makeText(context, R.string.settings_suggest_success, Toast.LENGTH_SHORT).show();
			progress.dismiss();
		}
	}
