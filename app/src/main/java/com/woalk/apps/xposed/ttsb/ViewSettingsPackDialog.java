...
public class ViewSettingsPackDialog extends DialogFragment {
	...
	public Dialog onCreateDialog(Bundle savedInstanceState) {
		...
		edit_childindexes = ((EditText) v
				.findViewById(R.id.editText_childindexes));
		...
		if (vsetpk.settings.padding != null) {
			...
			check_actionbar_h
					.setChecked(vsetpk.settings.padding.plus_actionbar_h);
		...
		builder.setView(v)
				.setPositiveButton(android.R.string.ok,
						new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog, int id) {
								vsetpk.from = spinnerFrom
										.getSelectedItemPosition();
								if (!edit_levels.getText().equals(""))
									vsetpk.levels = Integer
											.parseInt(edit_levels.getText()
													.toString());
								if (!edit_childindexes.getText().equals(""))
									vsetpk.setChildIndexesFromString(edit_childindexes
											.getText().toString());
								else
								...
								if (checkFSW.isChecked()) {
									...
									vsetpk.settings.setFSW_value = radioFSW1
											.isChecked();
								} else {
								...
								if (checkCTP.isChecked()) {
									...
									vsetpk.settings.setCTP_value = radioCTP1
											.isChecked();
								} else {
								...
								if (checkPadding.isChecked()) {
									...
									if (!editLeft.getText().equals("")
										...
										try {
											vsetpk.settings.padding.left = Integer
													.valueOf(editLeft.getText()
															.toString());
											vsetpk.settings.padding.top = Integer
													.valueOf(editTop.getText()
															.toString());
											vsetpk.settings.padding.right = Integer
													.valueOf(editRight
															.getText()
															.toString());
											vsetpk.settings.padding.bottom = Integer
													.valueOf(editBottom
															.getText()
															.toString());
									...
									vsetpk.settings.padding.plus_status_h = check_status_h
											.isChecked();
									vsetpk.settings.padding.plus_actionbar_h = check_actionbar_h
											.isChecked();
									vsetpk.settings.padding.plus_nav_h = check_nav_h
											.isChecked();
									vsetpk.settings.padding.plus_nav_w = check_nav_w
											.isChecked();
								} else {
				...
				.setNegativeButton(android.R.string.cancel,
						new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog, int id) {
								ViewSettingsPackDialog.this.getDialog()
										.cancel();
